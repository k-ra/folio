...
Without these settings the editor remains local-only and the existing info panel explains that cloud accounts are unconfigured.
...
- Conflicts are surfaced in the info panel. **Keep both copies** retains the device version as a new story and the remote version separately (or retains the device copy separately when the remote was deleted). There is no silent last-writer-wins or automatic text merge.
...
All actions are inside the existing info/history panel. Account/import controls are also available in the existing homepage settings panel, so an empty library can sign in or import without creating a story. No new page-level controls are added.

- **Copy entire story** copies the title, text/fancy blocks and visible captions in order with paragraph breaks; excludes private notes, prompts and chats.
...
- **Download HTML + assets (.zip)** puts larger media data URLs in an `assets/` directory with `index.html`; extract and keep them together. Fonts and executable code stay inline. This is available for every story, not only above an arbitrary threshold.
...
- Exports do not fetch anything. Images uploaded/generated inside Folio are embedded. Pre-existing remote/blob URLs and external resources referenced inside custom code are disclosed, not silently downloaded or promised to work offline. Sandboxed generated graphics still cannot fetch external resources. References requiring external libraries must already be self-contained to work. Source links remain ordinary outbound citations. `.folio` retains external URLs as references, not as magically recovered files.
