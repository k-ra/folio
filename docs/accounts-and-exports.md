...
- Index clippings are private rich text inside each story's JSON row. They use the same local-first save status, cloud revision checks, failure recovery and account ownership as the essay. No additional Supabase table, migration, service or credential is needed. `.folio` backups include Index; HTML/ZIP publication and Copy text exclude it, even when margin notes are included. Index changes do not create essay-history entries, and restoring an essay revision leaves Index intact.
- Opening a story carries over that workspace's earlier browser-only Index if no structured Index exists yet. Old v2/v3/v4 copies remain untouched. An existing cloud Index, including an intentionally empty one, wins over legacy browser data; nothing silently merges or overwrites it. Explicit browser-story import (including the labeled Google flow) also carries over legacy clippings for imported stories. Ordinary sign-in never reads guest clippings into the account. Corrupt legacy clippings stop that import rather than being silently omitted.
...
- AI credentials are optionally remembered in this browser's localStorage, separately for each signed-in account or guest. They do not sync with Google or Supabase; **Forget** removes the current account's saved key. Auth tokens are managed separately by Supabase Auth. Neither is part of story data, cloud payloads, or portable files. `.folio` import uses an allowlisted schema, including history snapshots.
