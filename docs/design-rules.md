...
- The homepage settings orb shows the API-key field directly beneath the homepage appearance controls. Inside a story, the utility beside save status opens the same form in a native dialog. Keys stay in tab memory; connecting must not generate or rewrite anything. Static-only hosts explain why AI is unavailable instead of showing a nonfunctional key field.
