...
- AI connection is an optional page utility beside the save status, using the existing header layout. Its native dialog traps focus and holds keys only in tab memory; connecting must not generate or rewrite anything. It is absent on static-only hosts.
