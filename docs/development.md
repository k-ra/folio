...
- `server/`: local Vite development middleware. Not a production server and not included in static hosting.
...
Current limitations: cloud sync, reader publishing and a production AI backend are not implemented. Style-prompt/home-theme helpers and explicit design samples are offline prototypes. D3/Three runtime packs remain [proposed](magic-graphics-research.md), not installed. For bringing your own API key or hosting static builds, see [hosting](hosting.md).
