...
**No API-key environment variable is required for the public site.** Each visitor chooses **Configure** on the homepage (or **Connect AI** inside a story) and enters their own OpenAI API key. It is held only in that tab's JavaScript memory, never IndexedDB, localStorage, sessionStorage, a cookie or a URL. Reloading or choosing **Disconnect AI** removes it. Connecting does not validate the key or call a paid model; the first explicit generation/chat request checks access.
