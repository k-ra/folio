...
**No API-key environment variable is required for the public site.** Each visitor opens the homepage settings orb, chooses **AI**, and enters their own OpenAI API key underneath (or opens the **AI / NO AI** control inside a story). It is held only in that tab's JavaScript memory, never IndexedDB, localStorage, sessionStorage, a cookie or a URL. Reloading or choosing **No AI** removes it. Saving a key does not validate it or call a paid model; the first explicit generation/chat request checks access.
...
After deploying, check `/api/magic/status`: it should report `byok: true` and `configured: false` without revealing any secret. Confirm a story saves across reload, AI settings open, and a reload forgets the key. Test one small live chat, graphic, image, text style and background deliberately with your own key; model-quality and account-access checks spend API usage and are not part of automated QA.
