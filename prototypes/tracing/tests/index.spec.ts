...
for (const width of [1440, 390])
  test(`index lives in the real editor drawer at ${width}px`, async ({ page }) => {
    ...
    const passage = chat.locator('.chat-message .folio-markdown p').nth(7)
    ...
    await save.click()
    const scroll = await chat.getByRole('log').evaluate((el) => el.scrollTop)
    ...
    await expect
      .poll(() => chat.getByRole('log').evaluate((el) => el.scrollTop))
    ...
    await page.getByRole('button', { name: 'Open chat', exact: true }).click()
    await chat.getByRole('button', { name: 'INDEX', exact: true }).click()
    await expect(index.locator('.index-excerpt')).toContainText(excerpt)
    ...
    await chat.getByRole('button', { name: 'INDEX', exact: true }).click()
    await expect(index.locator('.index-excerpt')).toContainText(excerpt)
    await expect(index.locator('.index-excerpt')).toContainText('actual editable')
