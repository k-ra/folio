...
export function stylePayload(request: StyleRequest) {
  return {
    ...
    instructions: [
      ...
      'For palette: return accessible six-digit hex background and ink colors. If asked for a gradient or atmospheric color field, also return CSS for it. Otherwise leave css empty. Preserve current colors for other categories.',
      'For page: return complete CSS for html and empty body using body::before/::after if needed. All CSS must use no HTML, scripts, URLs, imports, external fonts/resources or interface. Keep writing readable against the supplied ink. Motion only if requested. Graphics and data categories return empty css.',
