...
export default function InfoActions({ story }: { story?: Story }) {
  const { user, error: authError } = useAccount(),
    { cloud, importStory, flush, openChatSample } =
      useContext(WorkspaceActions);
  ...
  const act = async (fn: () => Promise<void> | void) => {
    ...
    try {
      ...
      setError(
        e instanceof Error
          ? e.message
          : "Could not finish. Your story is unchanged.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="info-actions">
      <div className="info-group">
        ...
        {!cloudClient ? (
          <small>
            Cloud accounts unavailable here. Stories still save locally.
          </small>
        ) : user ? (
          <>
            ...
            {cloud && (
              <>
                ...
                {!!cloud.browserCount && (
                  <>
                    <small>
                      {cloud.browserCount} browser stories remain on this
                      device.
                    </small>
                    <button
                      disabled={busy}
                      onClick={() => void act(cloud.importBrowser)}
                    >
                ...
                {cloud.importMessage && (
                  <small role="status">{cloud.importMessage}</small>
                )}
                {cloud.error && (
                  <div role="alert">
                ...
                {cloud.conflicts.map((c) => (
                  <div key={c.id} role="alert">
                    “{c.remote.story?.title || "Story"}” changed on another
                    device. Nothing was overwritten.
                    <button
                      disabled={busy}
                      onClick={() => void act(() => cloud.keepBoth(c))}
                    >
                      Keep both copies
                    </button>
                  </div>
          ...
          <>
            {import.meta.env.VITE_GOOGLE_AUTH_ENABLED === "true" && (
              <button
                disabled={busy}
                aria-label="Sign in with Google & import browser stories"
                title="Signs in and copies browser stories to your private cloud library. Originals remain here."
                onClick={() =>
                  void act(async () => {
                    await flush?.();
            ...
            {emailOpen && (
              ...
              >
                ...
                <button disabled={busy}>
                  {sent ? "Confirm sign-in" : "Email me a sign-in code"}
                </button>
                {sent && (
                  <button
                    type="button"
      ...
      {(importStory || openChatSample) && (
        <div className="info-secondary-actions">
          {importStory && (
            <>
              <button
                type="button"
              ...
              <input
                ref={backupInput}
                type="file"
                aria-label="Import .folio backup"
                hidden
                  ...
                  if (file)
                    void act(async () => {
                      if (file.size > 200 * 1024 * 1024)
                        throw new Error("Backup exceeds the 200 MB limit.");
                      importStory(readBackup(await file.text()));
                      setMessage(
                        "Backup imported. Existing stories are unchanged.",
                      );
                    });
                }}
              />
          ...
          {openChatSample && (
            <button onClick={openChatSample}>Open offline chat sample</button>
          )}
        </div>
      )}
