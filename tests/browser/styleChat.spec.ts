...
test('connecting from a style category never submits its drafted prompt', async ({ page }) => {
  ...
  await expect(panel.getByRole('button', { name: 'Create custom style' })).toBeEnabled()
  expect(calls).toBe(0)
  await expect(panel.getByLabel('Custom style message')).toHaveValue('An orbital garden')
  await panel.getByRole('button', { name: 'Create custom style' }).click()
  await expect(panel.getByRole('log')).toContainText(result.reply)
  expect(calls).toBe(1)
})
...
for (const width of [1440, 390]) {
  test(`custom styles refine, survive Apply/reload, and reach real-data generation at ${width}px`, async ({
    ...
    await panel.getByRole('button', { name: 'Refine style' }).click()
    ...
    await panel.getByRole('button', { name: 'Create custom style' }).click()
    await expect(panel.getByRole('log')).toContainText('A living particle world')
...
test('color conversation previews, resets, and closing revokes its changes', async ({ page }) => {
  ...
  await panel.getByRole('button', { name: 'Create custom style' }).click()
  await expect(panel.getByLabel('Ink color')).toHaveValue('#223344')
  await panel.getByRole('button', { name: 'Reset', exact: true }).click()
  await expect(panel.getByRole('log')).toHaveCount(0)
  await expect(panel.getByLabel('Ink color')).toHaveValue('#111111')
